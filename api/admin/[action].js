    ...
    switch (action) {
      ...
      case 'create-iteration':
        if (method === 'POST') {
          ...
          const { data: active } = await supabase.from('iterations').select('*').is('end_date', null).maybeSingle();
          if (active) {
      ...
      case 'org-data':
        if (method === 'GET') {
          ...
          const { data: iteration } = await supabase.from('iterations').select('*').eq('id', iteration_id).maybeSingle();
          if (!iteration) {
          ...
          const { data: roles } = await supabase.from('person_roles').select('*').eq('iteration_id', iteration_id);
      ...
      case 'db-view':
        if (method === 'GET') {
          ...
          if (!table) {
            return res.status(400).json({ success: false, message: 'Table name required' });
          }
          const allowed = ['people', 'iterations', 'organization_units', 'person_roles'];
