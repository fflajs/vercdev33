...
export default async function handler(req, res) {
  ...
  try {
    ...
    // -----------------------------
    if (action === 'active-iteration' && method === 'GET') {
    ...
    // -----------------------------
    if (action === 'create-iteration' && method === 'POST') {
    ...
    // -----------------------------
    if (action === 'close-iteration' && method === 'POST') {
      const { id } = req.body;
    ...
    // -----------------------------
    if (action === 'org-units' && method === 'POST') {
    ...
    // ORG DATA (GET)
    // -----------------------------
    if (action === 'org-data' && method === 'GET') {
      const iteration_id =
        req.query?.iteration_id ||
        new URL(req.url, `http://${req.headers.host}`).searchParams.get('iteration_id');
